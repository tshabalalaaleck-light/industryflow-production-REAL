...
export default function MachinesPage(){
  ...
  const addMachine=async()=>{
    ...
    const companyId = me?.role==="SUPER_ADMIN" ? (prompt("Enter Company ID for this machine (demo-company-123 or swasap-001):")||me.companyId) : me.companyId
    if(!companyId) return
  ...
  const changeStatus=async(id:string,newStatus:string)=>{ await fetch(`/api/machines?id=${id}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({id, status:newStatus})}); load() }
  const delMachine=async(id:string)=>{ if(!confirm("Delete machine?")) return; await fetch(`/api/machines?id=${id}`,{method:"DELETE"}); load() }
  ...
  <div style={{display:"flex", minHeight:"100vh", background:"#0a0a0f", color:"white", fontFamily:"sans-serif"}}>
    <div style={{width:260, background:"#111118", borderRight:"1px solid #222", padding:20, display:"flex", flexDirection:"column"}}>
      ...
      <div style={{marginTop:30, fontSize:10, color:"#666"}}>GENEALOGY</div>
      <a href="/dashboard" style={{color:"#666", fontSize:13, padding:"8px 0", display:"block", textDecoration:"none"}}>Dashboard</a>
      <div style={{marginTop:15, fontSize:10, color:"#666"}}>PRODUCTION</div>
      ...
      <a href="/dashboard/machines" style={{color:"#ff7a00", background:"#ff7a001a", borderLeft:"3px solid #ff7a00", padding:"8px 12px", borderRadius:6, fontSize:13, display:"block", textDecoration:"none", fontWeight:700}}>Machines</a>
      <a href="/dashboard/tenants" style={{color:"#666", fontSize:13, padding:"8px 0", display:"block", textDecoration:"none"}}>Companies</a>
      <div style={{marginTop:"auto", fontSize:11, color:"#666"}}>{me?.email}<br/><span style={{color:"#ff7a00"}}>{me?.role} • {me?.companyId}</span></div>
    </div>
    <div style={{flex:1, padding:24}}>
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"start"}}>
        <div><h1 style={{margin:0, fontSize:26, fontWeight:800}}>Machines</h1><p style={{margin:"4px 0 0", color:"#888", fontSize:13}}>{runningCount}/{machines.length} running • For {me?.role==="SUPER_ADMIN"? "All Companies" : me?.companyId}</p></div>
      </div>
      ...
      <div style={{marginTop:20, background:"#15151f", border:"1px solid #222", borderRadius:12, overflow:"hidden"}}>
        <div style={{display:"grid", gridTemplateColumns:"2fr 1.2fr 1fr 1fr 0.8fr 1.2fr 0.8fr", padding:"12px 16px", fontSize:10, color:"#666", textTransform:"uppercase", letterSpacing:"0.5px", borderBottom:"1px solid #222", background:"#111118"}}>
        ...
        {machines.map((m:any)=>{
          ...
          <div key={m.id} style={{display:"grid", gridTemplateColumns:"2fr 1.2fr 1fr 1fr 0.8fr 1.2fr 0.8fr", padding:"14px 16px", borderBottom:"1px solid #1a1a28", alignItems:"center", fontSize:13}}>
            <div style={{fontWeight:600}}>{m.name}<div style={{fontSize:10, color:"#666", marginTop:2}}>{m.id}</div></div>
            <div style={{color:"#ccc"}}>{m.category}</div>
            <div><span onClick={()=>{ const next = m.status==="Running"? "Idle" : m.status==="Idle"? "Maintenance":"Running"; changeStatus(m.id,next) }} style={{background:st.bg, color:st.color, border:st.border, padding:"4px 10px", borderRadius:12, fontSize:11, fontWeight:600, cursor:"pointer", display:"inline-flex", alignItems:"center", gap:5}}><span style={{width:6, height:6, borderRadius:"50%", background:st.dot, display:"inline-block"}}></span>{m.status}</span></div>
            ...
            <div style={{display:"flex", gap:6}}><button onClick={()=>delMachine(m.id)} style={{background:"none", border:"1px solid #333", color:"#666", borderRadius:4, padding:"3px 6px", fontSize:10}}>Del</button></div>
          </div>
          )
        })}
        {machines.length===0 && <div style={{padding:40, textAlign:"center", color:"#666"}}>No machines yet - Add your first machine</div>}
      </div>
      <div style={{marginTop:12, fontSize:11, color:"#666"}}>💡 Click status to cycle: Running → Idle → Maintenance • Work Number links machine to workflow/process</div>
    </div>
  </div>
  )
}
