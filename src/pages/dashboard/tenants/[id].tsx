...
export default function CompanyWorkspace(){
  ...
  useEffect(()=>{load()},[id])
  const addUser=async()=>{
    if(me?.role!=="SUPER_ADMIN"){ alert("Only Main Admin can add users!"); return }
    if(!email||!pwd) return alert("Email & Password required"); await fetch("/api/users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password:pwd,role:"USER",companyId:id,title})}); setEmail(""); setPwd(""); load()
  }
  const delUser=async(uid:string)=>{
    if(me?.role!=="SUPER_ADMIN"){ alert("Only Main Admin can remove users!"); return }
    if(!confirm("Delete user?")) return; await fetch(`/api/users?id=${uid}`,{method:"DELETE"}); load()
  }
  ...
  <div style={{display:"flex", minHeight:"100vh", background:"#0a0a0f", color:"white", fontFamily:"sans-serif"}}>
    <div style={{width:260, background:"#111118", borderRight:"1px solid #222", padding:20}}>
      ...
      <div style={{marginTop:20, fontSize:11, color:"#666"}}>{me?.email}<br/>{me?.title||me?.role}<br/>{isSuper?<span style={{color:"#ff7a00"}}>Main Admin - Full Access</span>:<span style={{color:"#888"}}>Management - View Only</span>}</div>
    </div>
    <div style={{flex:1, padding:28}}>
      <h1 style={{margin:0, fontSize:22}}>{company.name}</h1>
      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginTop:22}}>
        <div style={{background:"#15151f", border:"1px solid #222", borderRadius:12, padding:18}}>
          <h3 style={{marginTop:0, fontSize:14}}>Users ({users.length}) {isSuper? "- Main Admin Only" : "- View Only"}</h3>
          {isSuper ? <div style={{display:"flex", gap:6, marginBottom:14, flexWrap:"wrap"}}><input placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} style={{flex:1, minWidth:120, padding:"8px 10px", background:"#0a0a0f", border:"1px solid #333", color:"white", borderRadius:6, fontSize:12}}/><input placeholder="Pwd" value={pwd} onChange={e=>setPwd(e.target.value)} style={{width:80, padding:"8px 10px", background:"#0a0a0f", border:"1px solid #333", color:"white", borderRadius:6, fontSize:12}}/><select value={title} onChange={e=>setTitle(e.target.value)} style={{padding:"8px 10px", background:"#0a0a0f", border:"1px solid #333", color:"white", borderRadius:6, fontSize:11}}>{TITLES.map(t=><option key={t} value={t}>{t}</option>)}</select><button onClick={addUser} style={{padding:"8px 14px", background:"#ff7a00", color:"white", border:"none", borderRadius:6, fontSize:12}}>Add</button></div>
          : <div style={{background:"#1a1a24", border:"1px solid #333", padding:"8px 12px", borderRadius:6, fontSize:11, color:"#888", marginBottom:12}}>🔒 Only Main Admin can add/remove users. You have view access as {me?.title||"Management"}.</div>}
          {users.map((u:any)=><div key={u.id} style={{padding:"8px 0", borderBottom:"1px solid #1a1a28", fontSize:12, display:"flex", justifyContent:"space-between", alignItems:"center"}}><span>{u.email} <span style={{background:"#ff7a0022", color:"#ff7a00", padding:"2px 6px", borderRadius:4, fontSize:10, marginLeft:6}}>{u.title||"Data Capturer"}</span></span>{isSuper ? <button onClick={()=>delUser(u.id)} style={{background:"#ff333322", border:"1px solid #ff333344", color:"#ff5555", borderRadius:4, padding:"2px 6px", fontSize:10}}>Del</button> : <span style={{color:"#444", fontSize:10}}>View</span>}</div>)}
        </div>
        <div style={{background:"#15151f", border:"1px solid #222", borderRadius:12, padding:18}}><h3 style={{marginTop:0, fontSize:14}}>Workflows ({wfs.length})</h3><div style={{display:"flex", gap:8, marginBottom:14}}><input placeholder="Workflow name / Work Number" value={wfName} onChange={e=>setWfName(e.target.value)} style={{flex:1, padding:"8px 10px", background:"#0a0a0f", border:"1px solid #333", color:"white", borderRadius:6, fontSize:12}}/><button onClick={addWf} style={{padding:"8px 14px", background:"#ff7a00", color:"white", border:"none", borderRadius:6, fontSize:12}}>Add</button></div>{wfs.map((w:any)=><div key={w.id} style={{padding:"8px 0", borderBottom:"1px solid #1a1a28", fontSize:13}}><span style={{fontWeight:600, color:"#ff7a00"}}>{w.name}</span> - <span style={{fontSize:11, color:"#888"}}>{w.status}</span></div>)}</div>
      </div>
    </div>
  </div>
  )
