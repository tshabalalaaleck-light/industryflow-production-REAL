...
export default function CompanyWorkspace(){
  ...
  const [email,setEmail]=useState(""); const [pwd,setPwd]=useState(""); const [wfName,setWfName]=useState(""); const [me,setMe]=useState<any>(null); const [denied,setDenied]=useState(false)
  const load=async()=>{
    if(!id) return
  ...
  useEffect(()=>{load()},[id])
  const addUser=async()=>{ if(!email||!pwd) return; await fetch("/api/users",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email,password:pwd,role:"USER",companyId:id})}); setEmail(""); setPwd(""); load() }
  ...
  const printAll=()=>{ const html=`<html><body><h1>${company?.name} Production Sheet</h1><p>${new Date().toLocaleString()}</p>${wfs.map((w:any)=>`<p>${w.name} - ${w.status} - Notes:${w.notes?.map((n:any)=>n.text).join(",")}</p>`).join("")}</body></html>`; const win=window.open("","_blank"); win?.document.write(html); win?.document.close(); win?.print() }
  if(denied) return <div style={{minHeight:"100vh", background:"#0a0a0f", color:"white", display:"flex", alignItems:"center", justifyContent:"center"}}><div style={{background:"#15151f", padding:30, borderRadius:12, textAlign:"center", border:"1px solid #333"}}><h2>Access Restricted</h2><a href="/dashboard/tenants" style={{display:"inline-block", marginTop:10, background:"#ff7a00", color:"white", padding:"10px 20px", borderRadius:8, textDecoration:"none"}}>Back</a></div></div>
  ...
  <div style={{display:"flex", minHeight:"100vh", background:"#0a0a0f", color:"white", fontFamily:"sans-serif"}}>
    ...
    <div style={{flex:1, padding:28}}>
      <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}><h1 style={{margin:0, fontSize:22}}>{company.name}</h1><div style={{display:"flex", gap:8}}><button onClick={printAll} style={{padding:"10px 16px", background:"#ff7a00", color:"white", border:"none", borderRadius:8, fontSize:12, fontWeight:700}}>🖨️ Print Sheet</button><button onClick={()=>{ const s=`${company.name} Production`; const b=wfs.map((w:any)=>w.name).join("\n"); window.location.href=`mailto:?subject=${encodeURIComponent(s)}&body=${encodeURIComponent(b)}` }} style={{padding:"10px 16px", background:"#15151f", border:"1px solid #333", color:"#ccc", borderRadius:8, fontSize:12}}>📧 Email Sheet</button></div></div>
      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginTop:22}}>
        <div style={{background:"#15151f", border:"1px solid #222", borderRadius:12, padding:18}}><h3 style={{marginTop:0, fontSize:14}}>Users ({users.length})</h3><div style={{display:"flex", gap:8, marginBottom:14}}><input placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} style={{flex:1, padding:"8px 10px", background:"#0a0a0f", border:"1px solid #333", color:"white", borderRadius:6, fontSize:12}}/><input placeholder="Password" value={pwd} onChange={e=>setPwd(e.target.value)} style={{width:90, padding:"8px 10px", background:"#0a0a0f", border:"1px solid #333", color:"white", borderRadius:6, fontSize:12}}/><button onClick={addUser} style={{padding:"8px 14px", background:"#ff7a00", color:"white", border:"none", borderRadius:6, fontSize:12}}>Add</button></div>{users.map((u:any)=><div key={u.id} style={{padding:"8px 0", borderBottom:"1px solid #1a1a28", fontSize:13, display:"flex", justifyContent:"space-between"}}><span>{u.email}</span><span style={{color:"#666", fontSize:11}}>{u.role}</span></div>)}</div>
        <div style={{background:"#15151f", border:"1px solid #222", borderRadius:12, padding:18}}><h3 style={{marginTop:0, fontSize:14}}>Workflows ({wfs.length})</h3><div style={{display:"flex", gap:8, marginBottom:14}}><input placeholder="Workflow name" value={wfName} onChange={e=>setWfName(e.target.value)} style={{flex:1, padding:"8px 10px", background:"#0a0a0f", border:"1px solid #333", color:"white", borderRadius:6, fontSize:12}}/><button onClick={addWf} style={{padding:"8px 14px", background:"#ff7a00", color:"white", border:"none", borderRadius:6, fontSize:12}}>Add</button></div>{wfs.map((w:any)=><div key={w.id} style={{padding:"8px 0", borderBottom:"1px solid #1a1a28", fontSize:13}}><div style={{display:"flex", justifyContent:"space-between"}}><span style={{fontWeight:600, color:"#ff7a00"}}>{w.name}</span><span style={{fontSize:11, color:"#888"}}>{w.status}</span></div>{w.notes?.length>0 && <div style={{fontSize:11, color:"#aaa", marginTop:4}}>📝 {w.notes.length} notes</div>}</div>)}</div>
      </div>
    </div>
  </div>
  )
